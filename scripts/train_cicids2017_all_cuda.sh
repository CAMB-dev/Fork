...
case "${SPLIT}" in
  random_flow)
    SPLIT_DIR="${SPLIT_DIR:-data/processed/cicids2017/all_split_label_stratified}"
    ;;
  time_ordered)
    SPLIT_DIR="${SPLIT_DIR:-data/processed/cicids2017/all_split_time_ordered}"
    ;;
  time_block)
    SPLIT_DIR="${SPLIT_DIR:-data/processed/cicids2017/all_split_time_block}"
    ;;
  time_block_128)
    SPLIT_DIR="${SPLIT_DIR:-data/processed/cicids2017/all_split_time_block_128}"
    ;;
  custom)
    if [[ -z "${SPLIT_DIR:-}" ]]; then
      echo "SPLIT=custom requires SPLIT_DIR" >&2
      exit 1
    fi
    ;;
  *)
    echo "Unsupported SPLIT=${SPLIT}. Use random_flow, time_ordered, time_block, time_block_128, or custom." >&2
    exit 1
    ;;
esac

TRAIN_PATH="${TRAIN_PATH:-${SPLIT_DIR}/train.parquet}"
VAL_PATH="${VAL_PATH:-${SPLIT_DIR}/val.parquet}"
TEST_PATH="${TEST_PATH:-${SPLIT_DIR}/test.parquet}"
OUTPUT_ROOT="${OUTPUT_ROOT:-artifacts/cicids2017_all/${SPLIT}}"

require_path() {
  local path="$1"
  if [[ ! -e "${path}" ]]; then
    echo "Required path not found: ${path}" >&2
    echo "Regenerate CICIDS2017 splits first:" >&2
    echo "  FORCE=1 MAX_PER_MAJOR=50000 bash scripts/prepare_cicids2017_all_parallel.sh" >&2
    exit 1
  fi
}
...
echo "== CICIDS2017 training config =="
cat <<EOF
PLAN=${PLAN}
SPLIT=${SPLIT}
SPLIT_DIR=${SPLIT_DIR}
